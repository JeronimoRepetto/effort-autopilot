...
export const PROVIDER_SKILL_ROOTS = Object.freeze({
  codex: ".agents/skills",
...
export function renderProviderSkillIndex(skill, canonicalReference) {
  return `---
name: ${skill.name}
description: >
  ${skill.description}
...
Before performing this workflow, read and follow [the canonical \`${skill.name}\` skill](${canonicalReference}) completely.

The linked file is the only editable source of truth. Run \`npm run skills:sync\` after changing canonical Agent Skills.
`;
}

function providerIndexes(root, skills) {
  return Object.entries(PROVIDER_SKILL_ROOTS).flatMap(([provider, skillRoot]) =>
    skills.map((skill) => {
      const target = path.join(root, skillRoot, skill.name, "SKILL.md");
      const canonical = path.join(root, "agent-skills", skill.name, "SKILL.md");
      const canonicalReference = path
        .relative(path.dirname(target), canonical)
        .split(path.sep)
        .join("/");
      return Object.freeze({
        provider,
        target,
        content: renderProviderSkillIndex(skill, canonicalReference),
      });
    }),
  );
}
...
export async function syncAgentSkills({
  ...
  for (const currentScope of scopes) {
    ...
    previews.push({ kind: "scope", scope: currentScope, target, content: section });
    if (next === source) continue;
    changes.push(target);
    if (!check && !dryRun) await writeFile(target, next, "utf8");
  }

  const indexes = providerIndexes(root, skills);
  for (const index of indexes) {
    previews.push({ kind: "provider", ...index });
    const current = await readExisting(index.target);
    if (current === index.content) continue;
    changes.push(index.target);
    if (!check && !dryRun) {
      await mkdir(path.dirname(index.target), { recursive: true });
      await writeFile(index.target, index.content, "utf8");
    }
  }

  return Object.freeze({ skills, indexes, changes, previews });
}
...
async function main() {
  ...
  if (options.check && result.changes.length > 0) {
    ...
    throw new Error(
      `generated skill routing or provider indexes are stale: ${relative.join(", ")}`,
    );
  }
  const verb = options.check ? "validated" : result.changes.length > 0 ? "updated" : "unchanged";
  process.stdout.write(
    `Agent Skills ${verb}: ${result.skills.length} skills, ${result.indexes.length} provider indexes, ${result.changes.length} changed files.\n`,
  );
}
