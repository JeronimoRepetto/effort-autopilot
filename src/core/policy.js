...
export const LENGTH_BANDS = Object.freeze([
  ...
  { minWords: 31, weight: 1, name: "detailed", reason: "The request includes meaningful detail." },
  {
    ...
    weight: 2,
    name: "long",
    reason: "The request is long enough to imply several constraints.",
  },
  {
    minWords: 181,
    weight: 3,
    name: "very-long",
    reason: "The request contains extensive requirements.",
  },
]);
...
export const FEATURE_RULES = Object.freeze([
  {
    ...
    weight: -2,
    pattern:
      /\b(rename|format|reformat|spellcheck|typo|one[- ]line|single line|explain briefly|summarize briefly|list files|show status|renombra|renombrar|formatea|formatear|ortograf[ií]a|una l[ií]nea|explica brevemente|resume brevemente|lista (?:los )?archivos|muestra (?:el )?estado)\b/i,
  ...
  },
  {
    ...
    weight: -1,
    pattern:
      /\b(quick|quickly|brief|briefly|fast|minimal answer|no explanation|r[aá]pid[oa]|r[aá]pidamente|breve|brevemente|respuesta m[ií]nima|sin explicaci[oó]n)\b/i,
  ...
  },
  {
    ...
    weight: 1,
    pattern:
      /\b(implement|build|create|add|write|update|change|fix|patch|implementa(?:r)?|constru(?:ye|ir)|cr[eé]a(?:me|r)?|a[nñ]ad(?:e|ir)|agrega(?:r)?|escri(?:be|bir)|actualiza(?:r)?|cambia(?:r)?|arregla(?:r)?|corrige|corregir|parche)\b/i,
  ...
  },
  {
    ...
    weight: 2,
    pattern:
      /\b(debug|diagnos(?:e|is)|root cause|investigate|trace|race condition|deadlock|flaky|intermittent|depura(?:r)?|diagnostica(?:r)?|causa ra[ií]z|investiga(?:r)?|rastrea(?:r)?|condici[oó]n de carrera|interbloqueo|inestable|intermitente)\b/i,
  ...
  },
  {
    ...
    weight: 2,
    pattern:
      /\b(architect(?:ure|ural)?|design system|distributed system|concurrency|consensus|compiler|algorithm|protocol|state machine|arquitectura|arquitect[oó]nico|dise[nñ]a(?:r)?|sistema distribuido|concurrencia|consenso|compilador|algoritmo|protocolo|m[aá]quina de estados)\b/i,
  ...
  },
  {
    ...
    weight: 2,
    pattern:
      /\b(across the (?:entire )?(?:repo|repository|codebase)|cross[- ]platform|end[- ]to[- ]end|monorepo|multiple services|system[- ]wide|large[- ]scale|(?:en|por) todo el (?:repo|repositorio|c[oó]digo base)|multiplataforma|de extremo a extremo|m[uú]ltiples servicios|en todo el sistema|a gran escala)\b/i,
  ...
  },
  {
    ...
    weight: 1,
    pattern:
      /\b(test|tests|testing|verify|validation|benchmark|prove|reproduce|prueba|pruebas|probar|verifica(?:r)?|validaci[oó]n|demuestra|demostrar|reproduc(?:e|ir))\b/i,
  ...
  },
  {
    ...
    weight: 3,
    pattern:
      /\b(exhaustive|comprehensive audit|all edge cases|formal verification|threat model|production[- ]critical|zero downtime|exhaustiv[oa]|auditor[ií]a (?:integral|exhaustiva)|todos los casos l[ií]mite|verificaci[oó]n formal|modelo de amenazas|cr[ií]tico para producci[oó]n|sin tiempo de inactividad)\b/i,
...
export const SYSTEM_FEATURES = Object.freeze({
  ui: Object.freeze({
    pattern:
      /\b(user interface|UI|button|toggle|floating button|menu bar|tray icon|widget|interfaz(?: de usuario)?|bot[oó]n|interruptor|bot[oó]n flotante|barra de men[uú]|icono de bandeja|widget)\b/i,
  }),
  osIntegration: Object.freeze({
    pattern:
      /\b(operating system|system[- ]wide|OS[- ]level|computer|desktop app|background service|global hotkey|sistema operativo|en todo el sistema|ordenador|computadora|aplicaci[oó]n de escritorio|servicio en segundo plano|atajo global)\b/i,
  }),
  permissionDeviceControl: Object.freeze({
    pattern:
      /\b(permission|permissions|entitlement|system API|OS API|microphone|camera|audio device|input device|mute|unmute|device control|permiso|permisos|API del sistema|micr[oó]fono|c[aá]mara|dispositivo de audio|dispositivo de entrada|mutear|mutee|desmutear|desmutee|control de dispositivos)\b/i,
  }),
  multiDevice: Object.freeze({
    pattern:
      /\b(all|every) (?:the )?(?:microphones?|cameras?|audio devices?|input devices?)|\b(?:todos|todas) (?:los|las) (?:micr[oó]fonos|c[aá]maras|dispositivos de audio|dispositivos de entrada)\b/i,
  }),
...
export const UNCERTAINTY_RULES = Object.freeze([
  {
    ...
    pattern:
      /^(?:please\s+)?(?:fix|change|update|improve|review|handle)\s+(?:it|this|that)\s*[.!?]*$/i,
