...
Requirements: Node.js 20.11+. Claude Code 2.1.238 was used for the non-inference command audit, but normal tests do not launch it.
...
`.github/workflows/ci.yml` runs on every push to `main` and every pull request: `npm ci`, `npm test`, `npm run lint`, `npm run format:check`, and `npm run skills:check`, across Ubuntu, Windows, and macOS on Node 20.11, 22, and 24. Node 20.11 is the oldest version the shipped code supports (`import.meta.dirname`). `npm ci` builds `node-pty` from source on Linux and installs its shipped prebuilt binaries on Windows and macOS. The suite never launches the installed Claude CLI, so CI performs no billable inference.
