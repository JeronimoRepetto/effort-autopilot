...
- **Remaining release blocker:** macOS verification on real hardware (Linux is WSL-verified). The 2026-08-23 hardware runs surfaced an IPC socket-path overflow past the 104-byte `sun_path` limit (issue #18) and a broker-startup failure that aborted the launch instead of failing open (issue #19); both are fixed and regression-tested — the socket basename is short and length-guarded, and any broker setup failure now degrades to an unchanged Claude session with a visible `broker-setup-failed` cause — and both fixes were re-verified on hardware the same day (the live IPC tests and the PTY mock now pass on the MacBook). A third, environment-level finding was confirmed with evidence: pnpm v11 drops the execute bit on node-pty's prebuilt `spawn-helper` (`posix_spawnp failed`; workaround documented in TROUBLESHOOTING.md, product-level repair tracked in issue #26). Still open before the blocker clears: the hardware re-run of the now-green-in-code suite (the 4 platform-simulation failures were path helpers ignoring their `platform` parameter, fixed in issue #24) and the full live-launch proof.
