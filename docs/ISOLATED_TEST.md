...
At startup the broker prints one line naming the session's starting effort (taken from your local `effortLevel` setting, or `auto`); that pin only makes the starting level known so already-active levels are never re-sent — it does not change persistence (see the caveat below). If broker setup or the PTY transport fails after that line, the pin is not applied: a `broker-setup-failed` or `pty-spawn-failed` notice supersedes it and Claude runs with your original arguments. Mid-conversation escalations trigger the CLI's own `Change effort level?` confirmation, which the broker confirms itself.
...
A user-supplied `--settings` argument is combined additively (your hooks run first, the broker's are appended; everything else is preserved). A settings value that cannot be combined safely, `--print`, and `--resume`/`--continue` launches all print a visible notice and run Claude completely unchanged instead of degrading it. A broker setup failure does the same: a visible `broker-setup-failed` notice and Claude running completely unchanged with the original arguments. If the PTY transport itself cannot start, a final `pty-spawn-failed` notice appears and Claude runs directly attached to the real terminal — still your original arguments, no broker features. On macOS with pnpm-installed dependencies you may also see a one-line notice that the broker repaired node-pty's spawn-helper execute permission at startup; that is the expected self-heal, not an error.
