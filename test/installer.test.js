...
test("install paths are platform-correct and derived from one root", () => {
  const win = { platform: "win32", env: { LOCALAPPDATA: "C:\\Users\\u\\AppData\\Local" }, home: "C:\\Users\\u" };
...
test("PATH entry editing is idempotent, surgical, and case-insensitive on Windows", () => {
  ...
  assert.equal(prependPathEntry("/usr/bin:/bin", "/home/u/.effort-autopilot/shim", { separator: ":", caseInsensitive: false }), "/home/u/.effort-autopilot/shim:/usr/bin:/bin");
});
...
test("shim contents invoke the broker and forward all arguments", () => {
  const win = windowsShimContent("C:\\Program Files\\nodejs\\node.exe", "C:\\pkg\\bin\\internal-interactive-broker.js");
...
test("project config reads enabled/policy tolerantly and flags invalid documents", () => {
  ...
  assert.deepEqual(read(JSON.stringify({ enabled: false })), { enabled: false, policy: undefined, invalid: false });
  assert.deepEqual(read(`\uFEFF${JSON.stringify({ policy: "autopilot-wins" })}`), { enabled: undefined, policy: "autopilot-wins", invalid: false });
  assert.equal(read('{"policy": "turbo"}').policy, undefined);
  assert.equal(read("{ not json").invalid, true);
  const missing = readProjectConfig({ cwd: "C:\\proj", readFile: () => { throw new Error("ENOENT"); } });
...
test("policy resolution honors launch > project > global > default", () => {
  assert.deepEqual(
    resolveAutopilotPolicy({ launchPolicy: "manual-wins", projectPolicy: "autopilot-wins", globalPolicy: "autopilot-wins" }),
    { policy: "manual-wins", source: "launch flag" },
  );
  assert.deepEqual(
    resolveAutopilotPolicy({ launchPolicy: null, projectPolicy: "autopilot-wins", globalPolicy: "manual-wins" }),
    { policy: "autopilot-wins", source: "project config" },
  );
  assert.deepEqual(
    resolveAutopilotPolicy({ launchPolicy: null, projectPolicy: undefined, globalPolicy: "autopilot-wins" }),
...
test("global config read degrades to no-opinion when missing", () => {
  const missing = readGlobalConfig({ readFile: () => { throw new Error("ENOENT"); } });
